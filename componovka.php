...
$step=$_GET['step'];
if ($authlevel==1)
  {    print "<meta http-equiv='refresh' content='0; url=frames.php'>\n";
  }
else
  {    if ($step==1)
      {        print "<center>Процесс создания готового теста.</center>\n";
        ...
        print "</center></td>\n";
        print "<td><center>\n";
        print "<select id='block' size=\"1\" onChange='javascript:menu()' name='pid'>\n";
        print "<option value='' >Выбор предмета</option>\n";
    ...
    elseif ($step==2)
      {      	$pid=$_GET['pid'];
        print "<center>Процесс создания готового теста.</center>\n";
        print "<table align='center' valign='midle' cellpadding='7' width='70%'>\n";
        print "<form name='' action='glav.php?lnk=componovka&step=3' method='post'>\n";
         $q=mysql_query("SELECT * FROM `predmet`");
        print "<tr class='tr'>\n";
        print "<td><center>\n";
        print "Выберите предмет\n";
        ...
        if ($query=="")
          ...
          {            $nametest=mysql_query("SELECT * FROM `gotov_test` WHERE `name` = '$name'");
            ...
            if(! $nameerror == 0)
             ...
             {
               $q="INSERT INTO `gotov_test` VALUES (0, '$name', '$query', '$pid', '$uid')";               mysql_query($q);               print "<br /><br /><br /><hr>Ваш запрос успешно обработан...<hr>\n";               print "<meta http-equiv='refresh' content='3; url=glav.php'>\n";             }
          }
         }
      }
  }
