...
if($step==1) {  
  ...
  while($row=mysql_fetch_array($q))    {
    ...
    while($row1=mysql_fetch_array($getpredmet)) { 
      ...
      while($row2=mysql_fetch_array($getauthor)) { $authorname=$row2[4]; }
...
elseif($step==2) {
  ...
  {print "<br /><br /><center>Список допущенных к тестированию:</center><br /><br /><br />";}
  $q=mysql_query("SELECT * FROM `dostup`");
  $error=mysql_num_rows($q);
  if ($error==0) { print "<center><hr>Список допущеных пуст.<hr></center>"; }
  else { $css="class='shapka'";   
    {print "<table align='center' valign='midle' cellpaddind='7' width='70%'>\n    
            <tr $css>\n    
    ...
    while($row=mysql_fetch_array($q)) { 
      ...
      while($row3=mysql_fetch_array($getkto)) { $authorname=$row3[4]; }
      {print "<tr $css>\n       
